...
test.describe('native proposal form', () => {
  test('SL new race URL helpers, hidden links, exact payload, mobile and no console errors', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true }).fill('organizator.si/razpis');
    await page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true }).blur();
    await expect(page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true })).toHaveValue('https://organizator.si/razpis');
    await page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true }).fill('https://tekaski-koledar.si/dodaj-ali-popravi-tek/');
    await page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true }).blur();
    await expect(page.getByText('To je povezava na Slovenski Tekaški Koledar')).toBeVisible();
    await page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true }).fill('notaurl://bad');
    ...
    await expect(page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true })).toHaveAttribute('aria-invalid', 'true');
    await page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true }).fill('https://example.com/razpis/2026?zelo=dolg-url');
    await expect(page.getByText('Vnesite celoten spletni naslov')).toBeHidden();
    await expect(page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true })).not.toHaveAttribute('aria-invalid', 'true');
  ...
  test('EN prefilled correction keeps safe return URL, accessible names and reset cancel on desktop', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Official source (optional)', exact: true })).toHaveValue('https://example.com/zelo-dolg-url/razpis');
  ...
  test('combined existing change requires race identity, maps categories, and posts exact payload', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Kraj (neobvezno)', exact: true })).toBeVisible();
    await expect(page.getByRole('textbox', { name: 'Kraj (neobvezno)', exact: true })).not.toHaveAttribute('required', '');
  ...
  test('Other shows only relevant fields and posts exact payload without hidden blockers', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Kraj', exact: true })).toBeHidden();
    await expect(page.getByRole('combobox', { name: 'Regija' })).toBeHidden();
    await expect(page.getByRole('combobox', { name: 'Ali ste organizator?' })).toBeHidden();
    await expect(page.getByRole('combobox', { name: 'Ali je za izbrano leto že objavljen uradni razpis ali uradna objava?' })).toBeHidden();
    await expect(page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true })).toBeVisible();
  ...
  test('unsafe returnUrl is not rendered', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Uradni vir (neobvezno)', exact: true })).toHaveValue('');
  });
});
