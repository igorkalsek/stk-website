...
import { requiredProposalFields, isSafeInternalReturnUrl, readProposalPrefill, buildGoogleFormsFallbackUrl, getYearContext } from '../.cache/dist-test/proposal-form/proposal-form-controller.js';
...
describe('proposal form contract', () => {
  ...
  it('prefills correction query safely', () => {
    ...
    assert.equal(prefill.eventTitle, 'Šmarna gora'); assert.equal(prefill.safeReturnUrl, '/tek/2027/test/'); assert.match(prefill.description, /Year: 2027/);
  });
  it('accepts only internal returnUrl paths', () => { assert.equal(isSafeInternalReturnUrl('/tek/2026/a/'), true); assert.equal(isSafeInternalReturnUrl('//evil'), false); assert.equal(isSafeInternalReturnUrl('https://evil.test'), false); });
  ...
  it('preserves fallback prefill without storing private fields in URL/localStorage', () => {
    const url = buildGoogleFormsFallbackUrl(readProposalPrefill(new URLSearchParams('event=Tek&date=2026-03-02&place=Kranj&returnUrl=/tek/2026/tek/'), 'sl'));
    assert.match(url, /entry\.528776717=Tek/); assert.doesNotMatch(url, /entry\.600388817/);
