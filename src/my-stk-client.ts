...
export const initMyStk = async (root = document) => {
  ...
  content.innerHTML = `<div class="my-stk-heading"><h2>${language === 'en' ? 'My STK' : 'Moj STK'}</h2></div><div class="my-stk-dashboard"><article><span class="eyebrow">${language === 'en' ? 'My next race' : 'Naslednji moj tek'}</span>${nextCard}</article><article><span class="eyebrow">${language === 'en' ? `My ${DEFAULT_PUBLIC_YEAR} season` : `Moja sezona ${DEFAULT_PUBLIC_YEAR}`}</span><div class="my-stk-season-summary"><span class="season-metric"><strong>${summary.completedCount}</strong><span>${escapeHtml(completedLabel)}</span></span><span class="season-metric"><strong>${summary.regionCount} / ${totalRegions}</strong><span>${language === 'en' ? 'regions' : 'regij'}</span></span><span class="season-metric"><strong>${summary.distinctEventCount}</strong><span>${escapeHtml(distinctLabel)}</span></span></div><a class="button button-small button-secondary-light" href="${language === 'en' ? '/en/my-races/' : '/moji-teki/'}">${language === 'en' ? 'Open my season' : 'Odpri mojo sezono'}</a></article><article class="my-stk-discovery-card"><span class="eyebrow">${language === 'en' ? 'Exploring Slovenia' : 'Odkrivam Slovenijo'}</span><h3>${summary.regionCount} / ${totalRegions} ${language === 'en' ? 'regions' : 'regij'}</h3><progress max="${Math.max(totalRegions, 1)}" value="${summary.regionCount}" aria-label="${language === 'en' ? 'Regions explored' : 'Obiskane regije'}"></progress><p>${regionalCopy}</p><a class="button button-small button-secondary-light" href="${finder}" data-region-discovery>${language === 'en' ? 'Discover a race in a new region' : 'Odkrij tek v novi regiji'}</a></article></div>`;
