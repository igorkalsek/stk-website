...
export function formatPlanningWeekEventCount(kind: 'weekend' | 'weekday', count: number, lang: 'sl' | 'en'): string {
  const label = lang === 'en' ? (kind === 'weekend' ? 'Weekend' : 'Weekday') : (kind === 'weekend' ? 'Vikend' : 'Med tednom');
  return `${label}: ${formatPlanningEventCount(count, lang)}`;
}

export function formatPlanningWeekStatus(status: PlanningStatus, count: number, lang: 'sl' | 'en'): string {
  if (lang === 'en') {
